...
const string strMessageMagic = "BitQuark Signed Message:\n";
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitQuarkMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitQuarkMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("BitQuarkMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitquark-miner");
    ...
    try { while (true) {
        ...
        printf("Running BitQuarkMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("BitQuarkMiner terminated\n");
        throw;
    }
}
