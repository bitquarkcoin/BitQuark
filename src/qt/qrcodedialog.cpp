...
{
    QString ret = QString("bitquark:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BTQ unit input in lnReqAmount, we generate the URI with BTQ as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BTQ, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
